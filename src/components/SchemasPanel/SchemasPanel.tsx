...
import { AppButton, Badge, ConfirmDialog, EmptyState, ModalShell, PlusIcon, SchemaIcon } from '../ui'
...
function SchemaEditorModal({
  ...
  return (
    <ModalShell
      open
      title={state.mode === 'create' ? 'Add Schema' : `Edit Schema: ${state.originalName}`}
      size="xl"
      onClose={onClose}
      footer={(
        <>
          <AppButton variant="ghost" onClick={onClose}>Cancel</AppButton>
          <AppButton variant="primary" onClick={onSave}>Save Schema</AppButton>
        </>
      )}
    >
...
function RenameStrategyModal({
  ...
  return (
    <ModalShell
      ...
      size="md"
      onClose={onCancel}
      footer={<AppButton variant="ghost" onClick={onCancel}>Cancel</AppButton>}
    >
      <div className="space-y-4">
    ...
    mode === 'modal-only' ? (
      <>
        ...
        <ConfirmDialog
          open={!!deleteName}
          title="Delete Schema"
          description={deleteName ? `Delete schema ${deleteName}? This cannot be undone.` : ''}
          confirmLabel="Delete"
    ...
    <div className="p-8 bg-white flex-1 overflow-y-auto">
      <div className="max-w-4xl">
        <div className="flex items-center justify-between mb-1">
          ...
          <AppButton variant="primary" onClick={openCreate} leadingIcon={<PlusIcon className="h-4 w-4" />}>Add Schema</AppButton>
        </div>
        ...
        {schemaViews.length === 0 ? (
          <EmptyState
            icon={<SchemaIcon className="h-12 w-12" />}
            title="No schemas defined yet"
            description="Create your first reusable object model in components/schemas to reference it from request and response bodies."
            action={<AppButton variant="primary" onClick={openCreate} leadingIcon={<PlusIcon className="h-4 w-4" />}>Add Schema</AppButton>}
          />
        ) : (
          <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="status" tone={schema.editable ? 'success' : 'warning'}>{schema.editable ? 'Editable' : 'Read only'}</Badge>
                      </div>
                      {schema.editable ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {schema.editable && (
                        <AppButton size="sm" variant="primary" onClick={() => openEdit(schema.name)}>Edit</AppButton>
                      )}

                      <AppButton size="sm" variant="danger" onClick={() => setDeleteName(schema.name)}>Delete</AppButton>
                    </div>
                  </div>
